...
def extract_all_interviews():
    ...
    interviews_with_content = extract_content_for_links(all_interviews[:10])  # Test first 10
...
def extract_content_for_links(interviews):
    ...
    for i, interview in enumerate(interviews, 1):
        print(f"{i:3d}/{len(interviews)}: {interview['title'][:50]}...", end=" ")
        ...
        try:
            ...
            print(f"✓ ({len(content)} chars, {len(images)} images)")
        ...
        for platform, items in sorted(platforms.items()):
            ...
            for i, interview in enumerate(items, 1):
                ...
                if interview.get('extracted'):
                    f.write(f"     ✓ Content extracted ({interview['content_length']} chars)\n")
                    ...
                    f.write(f"     ✗ Content extraction failed\n")
...
<head>
    ...
    <script>
        ...
        function highlightSearchTerm(term) {{
            // Could extend to highlight search terms
        }}
    </script>
</head>
<body>
    ...
    <input type="text" id="search" class="search-box" placeholder="🔍 Search interviews by title or platform..." onkeyup="filterInterviews()">
